...
(describe "Minimax Player"
          ...
          (it "scores 10 plus depth when they win"
              (should= 14 (calculate-score win-board X 4)))

          (it "scores -10 minus depth when opponent wins"
              (should= -14 (calculate-score [O X nil O X nil O nil nil] X 4)))

          (it "scores 0 for a draw"
              (should= 0 (calculate-score draw-board X 0))
              )
...
;              (should= 4
;                       (choose-move [nil X nil O nil nil X O nil])))
;
